              ...
              <ul className="space-y-4">
                {initialContent?.whyInvestPoints?.map((item, idx) => (
                  <li key={idx} className="flex items-center gap-3 group">
                    ...
                    <p className="font-normal text-gray-700 leading-relaxed">{item}</p>
                  </li>
                )) ?? null}
              </ul>
            </motion.div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {initialContent?.investmentOptions?.map((opt, i) => (
              <div key={i} className="bg-off-white p-8 rounded-[1rem] border border-gray-100 hover:border-brand/20 transition-all group">
                <div className="text-2xl text-brand mb-6 font-bold">
                  {i === 0 ? '01' : i === 1 ? '02' : '03'}
                </div>
                <h3 className="font-semibold text-charcoal text-md tracking-tight mb-4">{opt.title}</h3>
                <ul className="space-y-3">
                  {opt.points?.map((p, j) => (
                    <li key={j} className="flex items-center gap-2 text-sm text-gray-500 font-medium leading-relaxed">
            ...
            {initialContent?.offerStack?.map((offer, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-8">
                    <h3 className={`text-xl font-bold uppercase tracking-tighter ${isElite ? 'text-white' : 'text-charcoal'}`}>
                      {offer.title}
                    </h3>
                  </div>
                  ...
                  <ul className="space-y-5 mb-10 flex-grow">
                    {offer.points?.map((p, j) => (
                      <li key={j} className={`flex items-start gap-3 text-sm font-medium leading-relaxed ${isElite ? 'text-white/70' : 'text-gray-500'}`}>
                        <CheckCircle2 size={18} className={`flex-shrink-0 mt-0.5 ${isElite ? 'text-white' : 'text-charcoal'}`} />
                        {p}
                      </li>
                    )) ?? null}
                  </ul>
