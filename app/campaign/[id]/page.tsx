...
export default function CampaignPropertyPage() {
  const { id } = useParams();
  ...
  if (!property) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-off-white">
        <h2 className="text-2xl font-bold text-charcoal mb-4">Property Not Found</h2>
        <button onClick={() => router.push('/campaign')} className="text-brand font-bold hover:underline">
          Return to Selection
        </button>
      </div>
    );
