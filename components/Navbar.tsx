...
const Navbar = () => {
  ...
  return (
    <>
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 py-3 px-6 bg-white shadow-md`}
      >
        <div className="w-full mx-auto flex justify-between items-center">
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <Link
                    href={link.href}
                    className={`relative px-4 py-2 font-medium text-sm transition-all duration-300 rounded-full hover:text-brand ${isActive ? 'text-brand bg-brand/5' : 'text-charcoal'
                      }`}
                  >
                    {link.name}
                    {isActive && (
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            >
              <Link
                href="/contact"
                className="bg-brand text-white px-6 py-2.5 rounded-sm text-sm font-bold hover:bg-brand-hover transition-all shadow-md hover:shadow-brand/20"
              >
                List Property
              </Link>
            </motion.div>
          ...
          <div className="flex items-center space-x-2 lg:hidden">
            <button
              onClick={() => setQrModalOpen(true)}
              className="p-2 text-charcoal hover:text-brand transition-colors cursor-pointer"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>
            </button>
            <button
              className="relative w-10 h-10 flex flex-col items-center justify-center space-y-1.5 focus:outline-none cursor-pointer"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              aria-label="Toggle menu"
            >
              <div className='text-neutral-800'>{mobileMenuOpen ? <X size={25} /> : <Menu size={25} />}</div>
            </button>
          </div>
        </div>
        {/* Mobile Menu */}
                  ...
                  {navLinks.map((link, i) => {
                    ...
                    return (
                      <Link
                        key={link.name}
                        href={link.href}
                        className={`flex items-center gap-3 text-sm font-medium px-4 py-3 rounded-2xl transition-all ${i === 0 ? 'mt-2' : ''} ${isActive ? 'text-brand bg-brand/5' : 'text-charcoal hover:bg-gray-50 hover:text-brand'
                          }`}
                        onClick={() => {
