  ...
  return (
    ...
    >
      ...
      <div className="relative h-64 w-full overflow-hidden shrink-0">
        {property.tag && (
          <div className="absolute top-4 left-4 z-10 bg-brand/90 backdrop-blur-sm px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest text-white">
            {property.tag}
          </div>
        )}
        <img
        ...
        <div className="absolute bottom-4 right-4 z-10 flex flex-col gap-1.5 items-end">
          {property.pricingOptions ? (
            property.pricingOptions.slice(0, 2).map((opt, i) => (
              <div key={i} className="bg-brand text-white px-3 py-1.5 rounded-xl text-[10px] font-bold shadow-lg backdrop-blur-md border border-white/10 uppercase tracking-tight">
                {opt.size}: {opt.price}
              </div>
            ))
          ) : (
      ...
      <div className="p-6 flex flex-col flex-grow text-center">
        ...
        <div className="mt-auto flex flex-col gap-4">
          <a 
            href={`https://wa.me/2348000000000?text=I'm interested in ${property.title}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 bg-[#25D366] text-white py-3 rounded-full font-bold hover:bg-[#128C7E] transition-all shadow-md active:scale-95 text-xs uppercase tracking-widest"
          >
